...
class PtySshProcessWrapper(process_base.ProcessWrapper):
    def __init__(self, command, working_directory, env_variables, ssh_data):
        super().__init__(command, working_directory, env_variables)
        ...
        # One of these might be replaced with just `self.pty_slave`
    ...
    def start_execution(self, command, working_directory):
        ...
        # ToDo: Decide the appropriate API or replacement for `subprocess.Popen`
        # 
        # I suspect `PtySshProcessWrapper` will become a multiplexer against 
        # [`paramiko.channel.Channel`](https://docs.paramiko.org/en/stable/api/channel.html#paramiko.channel.Channel)
        import socket

        tx_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        tx_sock.settimeout(10)
        tx_ip = self.ssh_data.get_host()
        tx_port = self.ssh_data.get_port()
        tx_sock.connect((tx_ip, tx_port))

        tx = paramiko.Transport(tx_sock)
        channel = tx.open_channel("session")

        stdin  = channel.makefile_stdin()
        output = channel.makefile_stderr()
